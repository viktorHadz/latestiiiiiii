export default function stylesManager() {
  return {
    ...
    async fetchSamples(clientId) {
      try {
        const response = await fetch(`/api/items/client/${clientId}`)
    ...
    async saveStyle(style) {
      try {
        ...
        if (response.ok) {
          ...
          Alpine.store('invoLocalStore').update('invoiceItems', this.invoiceItems)
        } else {
    ...
    async saveSample(sample) {
      try {
        ...
        const response = await fetch(`/api/items/${sample.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (confirm('Delete this style?')) {
        try {
          ...
          if (response.ok) {
            ...
            this.invoiceItems = this.invoiceItems.filter(item => !(item.id === styleId && item.type === 'style'))
            Alpine.store('invoLocalStore').update('invoiceItems', this.invoiceItems)
          } else {
    ...
    async deleteSample(sampleId) {
      if (confirm('Delete this sample?')) {
        try {
          const response = await fetch(`/api/items/${sampleId}`, {
            method: 'DELETE',
          })
          if (response.ok) {
            ...
            this.invoiceItems = this.invoiceItems.filter(item => !(item.id === sampleId && item.type === 'sample'))
            Alpine.store('invoLocalStore').update('invoiceItems', this.invoiceItems)
          } else {
